subroutine src2(meqn,mbc,mx,my,xlower,ylower,dx,dy,q,maux,aux,t,dt)
	...
	!----------------------------------
	! update q with source terms via q_t = psi
	!----------------------------------	
	do j = 1,mx
		do i = 1, mx
			q(3,i,j) = q(3,i,j)*(1. - dt/(Ma*dsqrt(E)))
			q(4,i,j) = q(4,i,j)*(1. - dt/(Ma*dsqrt(E)))
			q(5,i,j) = q(5,i,j)*(1. - dt/(Ma*dsqrt(E)))
		enddo
	enddo
	...
	call dgemv('n', mx*my, mx*my, beta, bu, mx*my, q_col(1,:), 1, 0.d0, bp, 1)
	
	!-- calculate beta*bu*u_star + beta*bv*v_starand store the result in bp
	call dgemv('n', mx*my, mx*my, beta, bv, mx*my, q_col(2,:), 1, 1.d0, bp, 1)

	!-- solve a*p = bp = beta*bu*u_star + beta*bv*v_star
	call dpbsv('u', mx*my, mx, 1, a_bs, mx + 1, bp, mx*my, info)

	!-- store the pressure distribution in q(6,:,:)
	do j = 1, my
		q(6,1:mx, j) = bp(((j-1)*mx+1):(j*mx))	
	enddo
	...
	do j = 1-mbc, mx+mbc
		do i = 1-mbc,mx
			u_edge(i,j) = u_edge(i,j) - dt/dx*(q(6,i+1,j) - q(6,i,j))
		enddo
	enddo
	...
	do j = 1-mbc, mx
		do i = 1-mbc,mx+mbc
			v_edge(i,j) = v_edge(i,j) - dt/dx*(q(6,i,j+1) - q(6,i,j))
		enddo
	enddo
	...
	do j = 1,mx 
		do i = 1,mx
			q(7,i,j) = (u_edge(i,j) - u_edge(i-1,j) + v_edge(i,j) - v_edge(i,j-1))/dx
		enddo
	enddo
	...
	do j = 1,mx
		do i = 1, mx
			q(1,i,j) = q(1,i,j) - dt/dx/2.d0*(q(6,i+1,j) - q(6,i-1,j))
			q(2,i,j) = q(2,i,j) - dt/dx/2.d0*(q(6,i,j+1) - q(6,i,j-1))
		enddo
	enddo
